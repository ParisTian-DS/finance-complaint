...
# For Transformation
# _transform -> Replace all categorcical value with freq
...
# a step need to follow when create a custom transformation
# can follow internal spark's transformer
class DerivedFeatureGenerator(Transformer, HasInputCols, HasOutputCols,
                              DefaultParamsReadable, DefaultParamsWritable):

    @keyword_only   #decorator provided by pyspark
    def __init__(self, inputCols: List[str] = None, outputCols: List[str] = None, ):
