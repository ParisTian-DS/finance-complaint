...
class DataIngestionArtifactData:
    ...
    def save_dataIngestion_artifact(self, data_ingestion_artifact: DataIngestionArtifact):
        self.collection.insert_one(data_ingestion_artifact.to_dict())

    def get_dataIngestion_artifact(self, query):
        self.collection.find_one(query)

    def update_dataIngestion_artifact(self, query, data_ingestion_artifact: DataIngestionArtifact):
        self.collection.update_one(query, data_ingestion_artifact.to_dict())

    def remove_dataIngestion_artifact(self, query):
        self.collection.delete_one(query)

    def remove_dataIngestion_artifacts(self, query):
        self.collection.delete_many(query)

    def get_dataIngestion_artifacts(self, query):
