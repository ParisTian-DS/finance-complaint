...
# download and upload model from/to cloud
class S3Estimator(CloudEstimator):
    ...
    def save(self, model_dir, key):
        ...
        Args:
            model_dir: Model dir to compress
            key: cloud storage key where model will be saved, just a directory path
        ...
        save_model_path = self.__get_save_model_path(key=key) # for prepare a new key
        self.s3_client.upload_file(model_zip_file_path, self.bucket_name, save_model_path) # from boto3 library for AWS
...
class AzureBlobEstimator(CloudEstimator):...


# make prediction using the local, latest model
...
class S3FinanceEstimator(FinanceComplaintEstimator, S3Estimator):
    ...
    # find the latest model from S3 bucket
    @property
...
if __name__ ==  "__main__":
    model = S3FinanceEstimator()
    model.get_latest_model_path()
    model.transform(dataframe)

## When using heritage class, it will first check itself's function, then it is not exist, it will check parent class
"""
